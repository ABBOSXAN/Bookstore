...
@click.option(
    ...
    default=True,
    help="Override variables from the .env file with those from the .env file.",
)
@click.argument('commandline', nargs=-1, type=click.UNPROCESSED)
def run(ctx: click.Context, override: bool, commandline: List[str]) -> None:
    """Run command with .env variables present."""
    file = ctx.obj['FILE']
...
def run_command(command: List[str], env: Dict[str, str]) -> int:
    ...
    added in the current .env variables.
    ...
    """
    # copy the current .env variables and add the vales from
    # `env`
